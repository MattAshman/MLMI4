...
class SGP_Layer(Base_SGP_Layer):
    ...
    def forward_prop_thru_cav_linearMean(self, mx, vx=None, final_layer=False, mode=PROP_MM):

        if vx is None:
            return self._forward_prop_deterministic_thru_cav_linearMean(mx)
        else:
            if mode == PROP_MM:
                if final_layer == False:
                    return self._forward_prop_random_thru_cav_mm_linearMean(mx, vx)
                else:
                    return self._forward_prop_random_thru_cav_mm(mx, vx)
            else:
                raise NotImplementedError('unknown propagation mode')
    ...
    def _forward_prop_deterministic_thru_cav_linearMean(self, x):
        """Propagate deterministic inputs thru cavity
        ...
        kff = np.exp(2 * self.sf)
        kfu = compute_kernel(2 * self.ls, 2 * self.sf, x, self.zu)
        mout = np.dot(x, self.W) + np.einsum('nm,dm->nd', kfu, self.Ahat)
        Bkfukuf = np.einsum('dab,na,nb->nd', self.Bhat_det, kfu, kfu)
        vout = kff + Bkfukuf
    ...
    @profile
    def _forward_prop_random_thru_cav_mm_linearMean(self, mx, vx):
        """Propagate uncertain inputs thru cavity, using simple Moment Matching
        ...
        psi1, psi2, check = compute_psi_weave_new(
            2 * self.ls, 2 * self.sf, mx, vx, self.zu)
        #check is NxMxQ which is (zmq*Snq+munq*lq)/(Snq+lq)
        psi3 = np.einsum('nmq,qd->dnm', check, self.W) * psi1[None,:,:]
        psi4 = np.einsum('nq,qd->nd', vx+mx**2, self.W**2)
        mout = np.einsum('nq,qd->nd', mx, self.W) + np.einsum('nm,dm->nd', psi1, self.Ahat)
        Bhatpsi2 = np.einsum('dab,nab->nd', self.Bhat_sto, psi2)
        Ahatpsi3 = np.einsum('dm,dnm->nd', self.Ahat, psi3)
        vout = psi0 + psi4 + 2*Ahatpsi3 + Bhatpsi2 - mout**2

        return mout, vout, psi1, psi2, psi3, psi4
    ...
    @profile
    def backprop_grads_lvm_mm_linearMean(self, m, v, dm, dv, psi1, psi2, psi3, psi4, mx, vx, alpha=1.0):
        """Summary
        ...
        # compute grads wrt Ahat and Bhat
        dm_all = dm - 2 * dv * m
        dAhat = np.einsum('nd,nm->dm', dm_all, psi1) + np.einsum('nd,dnm->dm', dv, psi3)
        dBhat = np.einsum('nd,nab->dab', dv, psi2)
        # compute grads wrt psi1 and psi2
        dpsi1 = np.einsum('nd,dm->nm', dm_all, self.Ahat)
        dpsi2 = np.einsum('nd,dab->nab', dv, self.Bhat_sto)
        dpsi3 = (dv.T)[:,:,None]*self.Ahat[:,None,:] #DxNxM
        dpsi4 = dv
        dsf2, dls, dzu, dmx, dvx = compute_psi_derivatives_new(
            dpsi1, psi1, dpsi2, psi2, dpsi3, psi3, dpsi4, ls, sf2, mx, vx, self.zu, self.W)
        dmx += np.einsum('nd,qd->nq', dm_all, self.W)
        ...
        dzu -= np.einsum('dm,qd->mq', dmcav, self.W)
        dvcav_via_mcav = beta * np.einsum('da,db->dab', dmcav, self.theta_2)
        dvcav += dvcav_via_mcav
        dvcavinv = - np.einsum('dab,dbc,dce->dae', Suhat, dvcav, Suhat)
        dtheta1 = beta * dvcavinv
        ...
        dKuuinv_via_Ahat = np.einsum('da,db->ab', dAhat, muhat-ZW)
        KuuinvSmmd = np.einsum('ab,dbc->dac', Kuuinv, Spmmhat)
        dKuuinv_via_Bhat = 2 * np.einsum('dab,dac->bc', KuuinvSmmd, dBhat) \
            - np.sum(dBhat, axis=0)
        dKuuinv = dKuuinv_via_Ahat + dKuuinv_via_Bhat + dKuuinv_via_vcav
    ...
    @profile
    def backprop_grads_reg_linearMean(self, m, v, dm, dv, kfu, x, alpha=1.0):
        """Summary
        ...
        dzu -= np.einsum('dm,qd->mq', dmucav, self.W)
        # add in contribution from the normalising factor
        Suinvmuhat = np.einsum('dab,db->da', Suhatinv, muhat)
        dmucav += scale_cav * Suinvmuhat
        dSucav += scale_cav * (
        ...
        dAhat = np.einsum('nd,nm->dm', dm, kfu)
        ZW = np.einsum('mq,qd->dm', self.zu, self.W)
        dKuuinv_m = np.einsum('da,db->ab', dAhat, muhat-ZW)
        KuuinvSmmd = np.einsum('ab,dbc->dac', Kuuinv, Suhat)
        dBhat = np.einsum('nd,na,nb->dab', dv, kfu, kfu)
        dKuuinv_v = 2 * np.einsum('dab,dac->bc', KuuinvSmmd, dBhat) \
            - np.sum(dBhat, axis=0)
    ...
    def compute_cavity(self, alpha):
        ...
        if self.nat_param:
            ...
            #print(self.Suhat.shape, "Suhat shape")
            self.muhat = np.einsum('dab,db->da', self.Suhat, beta * self.theta_2)
            #print(self.muhat.shape, "muhat shape")
        else:
    ...
    def compute_cavity_linearMean(self, alpha):
        ...
        ZW = np.einsum('mq,qd->dm', self.zu, self.W)
        self.Ahat = np.einsum('ab,db->da', Kuuinv, self.muhat-ZW)
        #print(self.Ahat.shape, "Ahat shape")
        Smm = self.Suhat + np.einsum('da,db->dab', self.muhat, self.muhat)
        #print(Smm.shape, "Splusmmhat shape")
        self.Splusmmhat = Smm
        SmmB = self.Suhat + np.einsum('da,db->dab', self.muhat-ZW, self.muhat-ZW)
        self.Bhat_sto = - Kuuinv + np.einsum(
            'ab,dbc->dac',
            Kuuinv,
            np.einsum('dab,bc->dac', SmmB, Kuuinv))
        #print(self.Bhat_sto.shape, "Bhat shape")
        self.Bhat_det = - Kuuinv + np.einsum(
            'ab,dbc->dac',
            Kuuinv,
...
class SDGPR(Base_SDGPR):
    ...
    def objective_function(self, params, mb_size, alpha=1.0, prop_mode=PROP_MM):
        ...
        for layer in self.sgp_layers[:-1]:
            layer.compute_cavity_linearMean(alpha)
        self.sgp_layers[-1].compute_cavity(alpha)
        #print("cavity over!")
        

        # propagate x cavity forward
        mout, vout, psi1, psi2, psi3, psi4 = [], [], [], [], [], []
        for i in range(0, self.L):
            layer = self.sgp_layers[i]
            if i == 0:
                # first layer
                m0, v0, kfu0 = layer.forward_prop_thru_cav_linearMean(xb)
                mout.append(m0)
                vout.append(v0)
                psi1.append(kfu0)
                psi2.append(None)
                psi3.append(None)
                psi4.append(None)
                #print("first layer over")
                
            elif i == self.L - 1:
                mi, vi, psi1i, psi2i = layer.forward_prop_thru_cav_linearMean(
                    mout[i - 1], vout[i - 1], final_layer=True)
                mout.append(mi)
                vout.append(vi)
                psi1.append(psi1i)
                psi2.append(psi2i)
                psi3.append(0.0)
                psi4.append(0.0)
                #print("last layer over")
                
            else:
                mi, vi, psi1i, psi2i, psi3i, psi4i = layer.forward_prop_thru_cav_linearMean(
                    mout[i - 1], vout[i - 1])
                mout.append(mi)
                vout.append(vi)
                psi1.append(psi1i)
        ...
        for i in range(self.L - 1, -1, -1):
            layer = self.sgp_layers[i]
            if i == 0:
                grad_hyper = layer.backprop_grads_reg_linearMean(
                ...
                grad_hyper, grad_input = layer.backprop_grads_lvm_mm_linearMean(
                    mout[i], vout[i], dmi, dvi, psi1[i], psi2[i], psi3[i], psi4[i],
                    mout[i - 1], vout[i - 1], alpha)
                dmi, dvi = grad_input['mx'], grad_input['vx']
    ...
    def objective_function_old(self, params, mb_size, alpha=1.0, prop_mode=PROP_MM):
        """Summary
