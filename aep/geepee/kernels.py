...
# add psi3 = E(KhumhathT)
def compute_psi_weave_new(lls2, lsf2, xmean, xvar, z):
    ls2 = np.exp(lls2)
    sf2 = np.exp(lsf2)
    sf2 = float(sf2)
    M = z.shape[0]
            ...
            for(int m2=0; m2<=m1; m2++) {
                ...
                for(int q=0; q<Q; q++) {
                    ...
                    if (m2==0) {
                        ...
                        check(n,m1,q) = (z1q*vq + xmean(n,q)*lq)/(vq + lq);
                    }
    ...
    weave.inline(code, support_code=support_code,
                 arg_names=['psi1', 'psi2', 'check', 'M', 'Q', 'N', 'sf2', 'ls2',
                            'z', 'xmean', 'xvar', 'log_denom_psi1', 'log_denom_psi2'],
                 type_converters=weave.converters.blitz)
    return psi1, psi2, check
...
def compute_psi_derivatives_new(dL_dpsi1, psi1, dL_dpsi2, psi2, dL_dpsi3, psi3, dL_dpsi4, ls, sf2, xmean, xvar, z, W):
    _dL_dvar_1, _dL_dl_1, _dL_dZ_1, _dL_dmu_1, _dL_dS_1 = \
        psi1compDer(dL_dpsi1, psi1, sf2, ls, z, xmean, xvar)
    _dL_dvar_2, _dL_dl_2, _dL_dZ_2, _dL_dmu_2, _dL_dS_2 = \
        psi2compDer(dL_dpsi2, psi2, sf2, ls, z, xmean, xvar)
    _dL_dvar_3, _dL_dl_3, _dL_dZ_3, _dL_dmu_3, _dL_dS_3 = \
        psi3compDer(dL_dpsi3, psi1, psi3, sf2, ls, z, xmean, xvar, W)
    _dL_dmu_4, _dL_dS_4 = \
        psi4compDer(dL_dpsi4, xmean, W)

    return (_dL_dvar_1 + _dL_dvar_2 + _dL_dvar_3 , _dL_dl_1 + _dL_dl_2 + _dL_dl_3,
            _dL_dZ_1 + _dL_dZ_2 + _dL_dZ_3, 
            _dL_dmu_1 + _dL_dmu_2 + _dL_dmu_3 + _dL_dmu_4, 
            _dL_dS_1 + _dL_dS_2 + _dL_dS_3 + _dL_dS_4)
...
def psi4compDer(dL_dpsi4, mu, W):
    # here are the "statistics" for psi1
    # Produced intermediate results: dL_dparams w.r.t. psi4
    # _dL_dmu           NxQ
    # _dL_dS            NxQ

    _dL_dmu = 2 * dL_dpsi4.dot((W**2).T) * mu
    _dL_dS = dL_dpsi4.dot((W**2).T)

    return _dL_dmu, _dL_dS
...
def psi3compDer(dL_dpsi3, _psi1, _psi3, variance, lengthscale, Z, mu, S, w):
    # here are the "statistics" for psi3
    # Produced intermediate results: dL_dparams w.r.t. psi3
    # _dL_dvariance     1
    # _dL_dlengthscale  Q
    # _dL_dZ            MxQ
    # _dL_dmu           NxQ
    ...
    dpsi3_w = np.einsum('dnm,qd->nmq', dL_dpsi3, w)
    dpsi3_w_psi1_z3 = np.einsum('nmq,nm,mq->nq', dpsi3_w, _psi1, Z**3)
    dpsi3_w_psi1_z2 = np.einsum('nmq,nm,mq->nq', dpsi3_w, _psi1, Z**2)
    dpsi3_w_psi1_z1 = np.einsum('nmq,nm,mq->nq', dpsi3_w, _psi1, Z)
    dpsi3_w_psi1 = np.einsum('nmq,nm->nq', dpsi3_w, _psi1)
    _dL_dmu = ( dpsi3_w_psi1_z2 * S + \
                dpsi3_w_psi1_z1 * mu * (lengthscale2-S) + \
                dpsi3_w_psi1 * lengthscale2 * (S+lengthscale2-mu**2)) * denom**2
    _dL_dS = 0.5 * (dpsi3_w_psi1_z3 * S + \
                    dpsi3_w_psi1_z2 * mu * (-2.0*S + lengthscale2) + \
                    dpsi3_w_psi1_z1 * (S-2.0*lengthscale2) * (mu**2 - 1./denom) + \
                    dpsi3_w_psi1 * mu * lengthscale2 * (mu**2 - 3./denom)) * denom**3
    
    _dL_dl = (( dpsi3_w_psi1_z3 * S * lengthscale2 + \
                dpsi3_w_psi1_z2 * mu * lengthscale2 * (-2.0*S + lengthscale2) + \
                dpsi3_w_psi1_z1 * (mu**2 * lengthscale2 + S/denom) * (S-2.0*lengthscale2) + \
                dpsi3_w_psi1 * mu * lengthscale2 * (mu**2*lengthscale2 + 3.0*S/denom)) * denom**3/lengthscale).sum(0)
    
    _dL_dvar = (dL_dpsi3 * _psi3).sum() / variance
    _dL_dZ = - np.einsum('nmq,nm,nq->mq', dpsi3_w, _psi1, S*denom**2) * Z**2 +\
               np.einsum('nmq,nm,nq->mq', dpsi3_w, _psi1, mu*(S-lengthscale2)*denom**2) * Z +\
               np.einsum('nmq,nm,nq->mq', dpsi3_w, _psi1, (mu**2*lengthscale2+S*(S+lengthscale2))*denom**2)
