...
class Probit_Layer(Lik_Layer):
    ...
    def output_probabilistic(self, mf, vf, alpha=1.0):
        ...
        return p, p - np.square(p)
    
    
class Multiclass_Layer(Lik_Layer):
    """Summary
    """
    __gh_points = None
    ...
    def __init__(self, N, D):
        ...
        Args:
            ...
            D (TYPE): Description
        """
        super(Multiclass_Layer, self).__init__(N, D)
        self.C = D
        self.epsilon = 1e-3
    ...
    def compute_log_Z(self, mout, vout, Y, alpha=1.0, compute_dm2=False):
        ...
        if mout.ndim == 2:
            if alpha == 1.0:
                ...
                v_der = (np.expand_dims(X, 1) - np.expand_dims(mout, 2)) / np.expand_dims(
                    np.sqrt(np.clip(vout, 1e-10, np.inf))**3, 2)
                ...
                Z = np.squeeze(np.dot(cdf_prod, gh_w))/ np.sqrt(np.pi) * (1 - self.C*self.epsilon/(self.C-1)) + self.epsilon/(self.C-1)
                logZ = np.sum(np.log(Z))
                
                Z = Z[:,np.newaxis]
                dZ_dm_off = np.squeeze(np.dot(cdf_prod[:,np.newaxis,:] * cdf_der, gh_w))\
                            /Z /np.pi /np.sqrt(2) * (1 - self.C*self.epsilon/(self.C-1))/ (-np.sqrt(np.clip(vout, 1e-10, np.inf)))\
                            * oh_off
                dZ_dm_on = np.dot(np.sum(cdf_der /np.expand_dims(np.sqrt(np.clip(vout, 1e-10, np.inf)),2), axis=1) * cdf_prod, gh_w)\
                            /Z /np.pi /np.sqrt(2) * (1 - self.C*self.epsilon/(self.C-1)) * oh_on
                dlogZ_dm = dZ_dm_off + dZ_dm_on
                
                dZ_dv_off = np.squeeze(np.dot(cdf_prod[:,np.newaxis,:] * cdf_der * v_der, gh_w))\
                            /Z /np.pi /np.sqrt(2) * (1 - self.C*self.epsilon/(self.C-1))/ (-2)\
                            * oh_off
                        
                v_der_on = np.expand_dims(gh_x / np.reshape(np.sqrt(np.clip(2. * var_selected, 1e-10, np.inf)), (-1, 1)),1)\
                            /np.expand_dims(np.sqrt(np.clip(vout, 1e-10, np.inf)), 2) /np.sqrt(2)
                dZ_dv_on = np.dot(np.sum(cdf_der * v_der_on, axis=1) * cdf_prod, gh_w)\
                            /Z /np.pi /np.sqrt(2) * (1 - self.C*self.epsilon/(self.C-1)) * oh_on
                    
                dlogZ_dv = dZ_dv_off + dZ_dv_on
            else:
                raise RuntimeError('invalid alpha=%d' % alpha)
    ...
    def output_probabilistic(self, mf, vf, alpha=1.0):
        ...
        def _predict_non_logged_density(self, mout, vout, Y):
            gh_x, gh_w = self._gh_points(GH_DEGREE)
            gh_w = gh_w[:, np.newaxis]
