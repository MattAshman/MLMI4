    ...
    for i in range(args.splits):
        ...
        for l in range(args.num_layers):
            ...
            if args.ard:
                ...
                kernels.append(SquaredExponential(lengthscale=[1.]*dim) +
                               White(variance=1e-5))
            else:
                # SE kernel with single lengthscale
                kernels.append(SquaredExponential(lengthscale=1.) +
                               White(variance=1e-5))
        ...
        t0 = time.time()
        # training loop
        monitored_training_loop(dgp_model, train_dataset, optimiser=optimiser,
                                logdir=args.log_dir,
        ...
        # nll
        test_nll = np.mean(logsumexp(norm.logpdf(Ys * Y_std, mean_SND *
                                                 Y_std,  var_SND ** 0.5 *
                                                 Y_std),
                                     0, b=1 / float(args.test_samples)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_samples', type=int, default=1,
                        help='Number of samples to propagate during training.')
